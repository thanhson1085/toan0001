...
//add replace on fileObj.filePath.replace("/wordpress/wp-admin/","") to workaround on windows server
	...
	test=file;
	var params = 'myajax=1&filename='+ pro_upload_dir+"/"+file.name + '&task=pro_resize_image_and_add&album_id='+jQuery('#multi_album_id').val();
...
function on_queue_completed(file)
{
	...
	pro_refresh_images_table(pro_current_album_id);
...
function pro_refresh_albums_table()
{
	...
	//sleep(2000);
	jQuery.post('index.php', 'task=pro_get_albums_table', function(res)
	{
		pro_set_data_tables();
		if (res.status == 'ok') 
		{
...
function pro_refresh_images_table(album_id)
{
	...
	jQuery.post('index.php', 'task=pro_get_albums_images_table&album_id='+album_id, function(res)
	{
		pro_set_data_tables();
		if (res.status == 'ok') 
		{
...
function re_order()
{
	...
	if( task == 'pro_reorder_album' )
		jQuery('#albums_table tbody').html('<tr><td colspan="7" align="center"><img src="'+loading.src+'" alt="" /><br/>Please wait, refreshing albums</td></tr>');
	if( task == 'pro_reorder_image' )
	...
	jQuery.get('index.php?' + params, function(res)
	{
		if( task == 'pro_reorder_image' )
		{
			pro_refresh_images_table(album_id);
		}
		if( task == 'pro_reorder_album' )
			pro_refresh_albums_table();
...
var tpager = null;
var ipager = null;
function pro_set_data_tables()
{
	//add event to bulk actions
	jQuery('.bulk_form').live('submit', pro_submit_bulk);
...
function pro_submit_bulk()
{
	var task = this.task.value;
	var album_id = null;
	if( task == 'pro_bulk_delete_albums' || task == 'pro_bulk_enable_albums' || task == 'pro_bulk_disable_albums')
	{
	...
	else if( task == 'pro_bulk_delete_images' || task == 'pro_bulk_enable_images' || task == 'pro_bulk_disable_images' )
	{
...
{
	//preload images
	loading.src = pro_url + '/images/loading.gif';
...
/*
		var ops = {
					'uploader'		: pro_url + '/js/uploadify-2.1.4/uploadify.swf',
					'script'		: pro_url + '/js/uploadify-2.1.4/uploadify.php',
					'cancelImg' 	: pro_url + '/js/uploadify-2.1.4/cancel.png',
				    'folder'    	: pro_upload_dir,
				    'auto'			: true,
					'multi'			: true,
					'fileExt'        : '*.jpg;*.gif;*.png',
...
*/
		var swfu;
			...
			var settings = {
				...
				upload_url: pro_url+"/js/swfupload/js/upload.php",
				post_params: {"PHPSESSID" : "<?php echo session_id(); ?>","folder":pro_upload_dir, "secw":pro_secw, nopar: "n" + new Date().getTime()},
				...
				// Button settings
				button_image_url: pro_url+"/js/swfupload/js/TestImageNoText_65x29.png",
				button_width: "130",
				button_height: "29",
	...
	jQuery('.pro_delete_album').live('click', pro_delete_album);
	...
	jQuery('table#album_images .enable, table#albums_table .enable').live('click', pro_enable);
	jQuery('table#album_images .disable, table#albums_table .disable').live('click', pro_disable);
